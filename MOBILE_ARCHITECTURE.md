...
`GoogleService-Info.plist` + the notifications config plugin.

### 12.1 Wiring (notification-system completion)

- **Config**: `app.json` → `android.package: com.petcare.bytari` (matches the
  Firebase Android app), `android.googleServicesFile: ./google-services.json`
  (repo root — `/android` is generated by `expo prebuild` and gitignored), and
  the `expo-notifications` plugin (`defaultChannel: "default"`, colour). The
  file holds only the Firebase **client** config; Admin credentials stay in
  `server/.env`.
- **Android channel** `default` (HIGH importance) is created on app start
  (`ensureAndroidChannel`); the backend posts every push to that channel id.
- **iOS**: `getDevicePushTokenAsync` returns an APNs token, which the backend's
  direct-FCM sender cannot deliver to, so iOS registration is **skipped**
  (logged). Enabling iOS needs an FCM token on iOS
  (`@react-native-firebase/messaging` + `GoogleService-Info.plist` + an APNs
  key uploaded to Firebase).
- **Token rotation**: `onTokenRefresh` re-registers (backend upserts by token).
- **Foreground**: the OS banner shows, except a `CHAT_MESSAGE_RECEIVED` for the
  conversation currently open (`setActiveConversation`, set by the chat thread
  screen on focus). Realtime only invalidates inbox queries — it never renders
  a second banner, so push + realtime never double-present.
- **Taps** (foreground / background / cold start): `notificationHref` maps
  `type` + `data` ids → a route (falling back to `entityType`), and
  `data.notificationId` marks that row read.
- **Copy**: under Arabic the inbox shows the server text; other languages
  re-render system types from `notifications:types.<TYPE>`
  (`localizedNotificationText`). Admin / organization broadcasts are always
  verbatim. The OS push itself is Arabic (server-rendered).
- **Web**: in-app inbox + realtime unread count only; no browser push
  (`notificationService.web.ts` is a no-op stub by design).
