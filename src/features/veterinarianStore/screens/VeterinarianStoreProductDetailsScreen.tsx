...
import { useVeterinarianStoreCartMutations, useVeterinarianStoreProduct } from '../hooks';
import { formatAmount } from '../utils';

/** Route `/veterinarian-store/products/[productId]` — product detail + add to cart. */
export default function VeterinarianStoreProductDetailsScreen() {
  const theme = useTheme();
  const { t } = useTranslation('veterinarianStore');
  const toast = useToast();
  const { productId } = useLocalSearchParams<{ productId: string }>();

  const q = useVeterinarianStoreProduct(productId);
    ...
    addItem.mutate(
      { productId: product.id, quantity },
      {
        onSuccess: () => {
          ...
          router.push(Routes.veterinarianStoreCart);
        },
