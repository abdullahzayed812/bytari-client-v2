...
import { useVeterinarianStoreOrder } from '../hooks';
import { formatAmount } from '../utils';

/**
 * Route `/veterinarian-store/orders/[orderId]` — one order. With `?justPlaced=1`
 * (set right after checkout) it opens as the order-confirmation screen.
 */
export default function VeterinarianStoreOrderConfirmationScreen() {
  ...
  const q = useVeterinarianStoreOrder(orderId);
  const order = q.data;
          ...
          {isConfirmation ? (
            ...
            >
              <Button
                ...
                fullWidth
                onPress={() => router.replace(Routes.veterinarianStore)}
              />
              <TextButton
                label={t('confirmation.viewOrders')}
                onPress={() => router.replace(Routes.veterinarianStoreOrders)}
              />
            </View>
          ) : null}
        </>
