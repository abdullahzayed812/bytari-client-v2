...
import { useVeterinarianStoreCart, useVeterinarianStoreCartMutations } from '../hooks';

/** Route `/veterinarian-store/cart` — سلة المشتريات. */
export default function VeterinarianStoreCartScreen() {
  const theme = useTheme();
  const { t } = useTranslation('veterinarianStore');
  const toast = useToast();

  const q = useVeterinarianStoreCart();
  const { updateItem, removeItem, clear } = useVeterinarianStoreCartMutations();
  const cart = q.data;
        ...
        <>
          ...
          >
            ...
            <Button
              ...
              onPress={() => router.push(Routes.veterinarianStoreCheckout)}
            />
          </View>
        </>
      )}
