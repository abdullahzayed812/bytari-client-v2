...
import { useVeterinarianStoreOrders } from '../hooks';
import { formatAmount } from '../utils';

/** Route `/veterinarian-store/orders` — the shopper's order history. */
export default function VeterinarianStoreOrdersScreen() {
  const theme = useTheme();
  const { t } = useTranslation('veterinarianStore');
  const q = useVeterinarianStoreOrders();
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Card
              variant="outlined"
              padding="md"
              onPress={() => router.push(Routes.veterinarianStoreOrder(item.id))}
