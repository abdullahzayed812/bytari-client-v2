...
import { useVetStoreAdminOrder, useUpdateVetStoreOrderStatus } from '../hooks';

/** Route `/(app)/admin/veterinarian-store/orders/[orderId]` — order detail + status control. */
export default function VeterinarianStoreAdminOrderDetailScreen() {
  const theme = useTheme();
  const { t } = useTranslation('veterinarianStore');
  const toast = useToast();
  const { orderId } = useLocalSearchParams<{ orderId: string }>();

  const q = useVetStoreAdminOrder(orderId);
  const updateStatus = useUpdateVetStoreOrderStatus();
  const order = q.data;

  const setStatus = (status: VetStoreOrderStatus): void => {
