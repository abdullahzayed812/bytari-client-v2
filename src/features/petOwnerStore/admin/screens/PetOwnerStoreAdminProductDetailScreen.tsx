...
import {
  ...
  usePetStoreProductImagePresignProvider,
} from '../hooks';

/** Route `/(app)/admin/pet-owner-store/products/[productId]` — preview + manage. */
export default function PetOwnerStoreAdminProductDetailScreen() {
  const theme = useTheme();
  const { t } = useTranslation('petOwnerStore');
  const toast = useToast();
  const { productId } = useLocalSearchParams<{ productId: string }>();

  const q = usePetStoreAdminProduct(productId);
  const { deactivate, removeImage } = usePetStoreAdminProductMutations();
  const presign = usePetStoreProductImagePresignProvider(productId);
        ...
        right={
          product ? (
            <Button
              ...
              size="sm"
              variant="ghost"
              onPress={() => router.push(Routes.adminPetStoreProductEdit(product.id))}
            />
          ) : undefined
        }
      />
