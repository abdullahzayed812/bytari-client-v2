...
import { usePetOwnerStoreCart, usePlacePetOwnerStoreOrder } from '../hooks';
import {
...
/** Route `/pet-owner-store/checkout` — إتمام الطلب. Cash on Delivery only for now. */
export default function PetOwnerStoreCheckoutScreen() {
  const theme = useTheme();
  const { t } = useTranslation('petOwnerStore');
  const toast = useToast();

  const cart = usePetOwnerStoreCart();
  const place = usePlacePetOwnerStoreOrder();
  const [method, setMethod] = useState<PetStorePaymentMethod>('COD');
  ...
  const onSubmit = (values: CheckoutForm): void => {
    place.mutate(
      ...
      },
      {
        onSuccess: (order) =>
          router.replace({
            pathname: '/(app)/pet-owner-store/orders/[orderId]',
    ...
    <SafeAreaScreen>
      ...
      >
        ...
        <View style={{ rowGap: theme.spacing.md }}>
          ...
          {PET_STORE_PAYMENT_METHODS.map((m) => (
            <PaymentMethodOption
              key={m}
              method={m}
              selected={method === m}
              disabled={!PET_STORE_ENABLED_PAYMENT_METHODS.includes(m)}
