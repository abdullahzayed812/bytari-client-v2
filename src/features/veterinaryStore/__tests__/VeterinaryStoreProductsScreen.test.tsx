...
import VeterinaryStoreProductsScreen from '../screens/VeterinaryStoreProductsScreen';
import type { VeterinaryStoreProduct } from '../types';

jest.mock('expo-router', () => require('@/test-utils/routerMock').expoRouter);

const getOrg = jest.spyOn(organizationsApi, 'get');
const list = jest.spyOn(veterinaryStoreProductsApi, 'list');
...
const page = (items: VeterinaryStoreProduct[], total = items.length) => ({
  items,
  meta: { page: 1, pageSize: 20, total, totalPages: 1 },
});

describe('VeterinaryStoreProductsScreen (§5, §23, §29, §34)', () => {
  it('lists the store catalogue scoped to the route organizationId; add CTA → create route', async () => {
    ...
    renderWithProviders(<VeterinaryStoreProductsScreen />);
    ...
    expect(routerMock.push).toHaveBeenCalledWith('/(app)/organizations/o1/store-products/create');
  });
  ...
  it('tapping a card opens that product’s detail', async () => {
    ...
    expect(routerMock.push).toHaveBeenCalledWith('/(app)/organizations/o1/store-products/p1');
  });
});
