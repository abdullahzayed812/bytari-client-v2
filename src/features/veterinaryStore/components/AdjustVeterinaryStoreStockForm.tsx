...
import { buildAdjustVeterinaryStoreStockSchema, type AdjustVeterinaryStoreStockFormValues } from '../validation/schemas';

export interface AdjustVeterinaryStoreStockFormProps {
  currentStock: number;
  submitting: boolean;
  formError?: string | null;
  serverFields?: Record<string, string>;
  onSubmit: (values: AdjustVeterinaryStoreStockFormValues) => void;
}

/**
 * Signed stock-delta form (`POST .../store-products/:id/stock`). A negative delta that
 ...
 */
export function AdjustVeterinaryStoreStockForm({
  currentStock,
  submitting,
  formError,
  serverFields = {},
  onSubmit,
}: AdjustVeterinaryStoreStockFormProps) {
  const theme = useTheme();
  const { t } = useTranslation('veterinaryStore');
  const schema = useMemo(() => buildAdjustVeterinaryStoreStockSchema(t), [t]);

  const { control, handleSubmit } = useForm<AdjustVeterinaryStoreStockFormValues>({
