...
import { SHEEP_PRODUCTION_TYPE_ORDER } from '../constants';
import type { SheepProductionType } from '../types';
import { buildCreateSheepFarmSchema, type CreateSheepFarmFormValues } from '../validation/schemas';

export interface SheepFarmFormProps {
  defaultValues?: Partial<CreateSheepFarmFormValues>;
  submitting: boolean;
  formError?: string | null;
  serverFields?: Record<string, string>;
  onSubmit: (values: CreateSheepFarmFormValues, image: LocalFile | null) => void;
}
 ...
 */
const DEFAULTS: CreateSheepFarmFormValues = {
  name: 'مزرعة الأغنام النموذجية',
  location: 'بغداد',
  governorate: 'بغداد',
  sheepProductionType: 'MEAT',
  description: 'مزرعة لتربية الأغنام لإنتاج اللحوم.',
  address: 'حي الجامعة، شارع 14',
  capacity: '500',
  currentSheepCount: '320',
  contactName: '',
  contactPhone: '',
  contactEmail: '',
  termsAccepted: false,
...
/** "Add Sheep Farm" form — mirrors `PoultryFarmForm` exactly. RTL, RHF + zod, no API logic. */
export function SheepFarmForm({ defaultValues, submitting, formError, serverFields = {}, onSubmit }: SheepFarmFormProps) {
  const theme = useTheme();
  const { t } = useTranslation('sheepCattleFarm');
  const { t: tc } = useTranslation('common');
  const toast = useToast();
  const schema = useMemo(() => buildCreateSheepFarmSchema(t), [t]);
  const [image, setImage] = useState<LocalFile | null>(null);

  const { control, handleSubmit } = useForm<CreateSheepFarmFormValues>({
  ...
  const productionOptions: { value: SheepProductionType; icon: IconName; label: string }[] =
    SHEEP_PRODUCTION_TYPE_ORDER.map((v) => ({
      value: v,
      icon: 'paw-outline',
      label: t(`create.sheepProduction.${v}`),
    }));
  ...
  return (
    <View style={{ rowGap: theme.spacing.lg }}>
      ...
      <FormCard icon="people-outline" title={t('create.sections.capacity')}>
        ...
        <FormField control={control} name="currentSheepCount" label={t('create.fields.currentSheepCount')} keyboardType="number-pad" serverError={serverFields.currentSheepCount} />
      </FormCard>
