...
const optionalCount = z.string().trim().regex(/^\d{1,9}$/).optional().or(z.literal(''));

// --- create farm (Sheep) ------------------------------------------

export function buildCreateSheepFarmSchema(t: LivestockTFn) {
  return z.object({
    name: z.string().trim().min(1, t('create.errors.nameRequired')).max(160, t('create.errors.tooLong')),
    location: z.string().trim().min(1, t('create.errors.locationRequired')).max(200, t('create.errors.tooLong')),
    governorate: z
      .string()
    ...
    description: z.string().trim().max(2000, t('create.errors.tooLong')).optional().or(z.literal('')),
    ...
    contactName: z.string().trim().max(160, t('create.errors.tooLong')).optional().or(z.literal('')),
    contactPhone: z.string().trim().max(40, t('create.errors.tooLong')).optional().or(z.literal('')),
    contactEmail: z.string().trim().email(t('create.errors.emailInvalid')).optional().or(z.literal('')),
    termsAccepted: z.boolean().refine((v) => v === true, { message: t('create.errors.termsRequired') }),
  });
}
