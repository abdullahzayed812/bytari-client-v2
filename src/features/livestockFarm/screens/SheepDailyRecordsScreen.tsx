...
import type { SheepDailyRecord } from '../types';

/**
 * Route `/(app)/livestock/sheep/[organizationId]/sections/daily` — البيانات اليومية
...
export default function SheepDailyRecordsScreen() {
  ...
  const records = useSheepDailyRecords(orgId, batch?.id, {
    pageSize: 100,
    enabled: Boolean(batch),
  });
  const create = useCreateSheepDailyRecord(orgId, batch?.id ?? '');
  const update = useUpdateSheepDailyRecord(orgId, batch?.id ?? '');
  const remove = useDeleteSheepDailyRecord(orgId, batch?.id ?? '');
  const [formOpen, setFormOpen] = useState(false);
  const [editing, setEditing] = useState<SheepDailyRecord | null>(null);
  const [deleting, setDeleting] = useState<SheepDailyRecord | null>(null);
