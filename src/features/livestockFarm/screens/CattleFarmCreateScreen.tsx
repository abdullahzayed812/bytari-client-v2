...
import { CattleFarmForm } from '../components';
import { useCreateCattleFarm } from '../hooks';
import type { CreateCattleFarmInput } from '../types';
import type { CreateCattleFarmFormValues } from '../validation/schemas';
...
export default function CattleFarmCreateScreen() {
  ...
  const defaults: Partial<CreateCattleFarmFormValues> = {
    contactName: user ? `${user.firstName} ${user.lastName}`.trim() : '',
    contactPhone: user?.phone ?? '',
    contactEmail: user?.email ?? '',
  };

  const onSubmit = (values: CreateCattleFarmFormValues, image: LocalFile | null): void => {
    ...
    create.mutate(input, {
      onSuccess: async (organization) => {
        ...
        router.replace(Routes.cattleFarmDetail(organization.id));
      },
      onError: (error) => {
  ...
  return (
    <OrgFormLayout title={t('create.cattleTitle')}>
      <CattleFarmForm
