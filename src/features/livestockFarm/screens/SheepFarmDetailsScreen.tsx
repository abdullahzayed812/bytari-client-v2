...
export default function SheepFarmDetailsScreen() {
  ...
  const daily = useSheepDailyRecords(orgId, batch?.id, { enabled: Boolean(batch) });
  ...
  const name = detail.data?.name ?? t('details.sheepTitle');
                  ...
                  {daily.isLoading ? (
                    ...
                    <View style={{ flexDirection: 'row', columnGap: theme.spacing.sm }}>
                      {lastRecord ? (
                        ...
                        <LivestockDailyRecordEmptyCard dayIndex={1} onPress={canManage ? () => router.push(Routes.sheepFarmSection(orgId, 'daily')) : undefined} />
                      )}
                      <LivestockDailyRecordEmptyCard
                        dayIndex={lastIndex + 1}
                        date={lastRecord ? nextDayDate(lastRecord.recordDate) : undefined}
                        onPress={canManage ? () => router.push(Routes.sheepFarmSection(orgId, 'daily')) : undefined}
                      />
                    </View>
                  )}
                </View>
                ...
                <View style={{ rowGap: theme.spacing.sm }}>
                  {closedBatches.batches.map((b) => (
                    <LivestockBatchRow
                      ...
                      onPress={() => router.push(Routes.sheepBatchDetail(orgId, b.id))}
                    />
                  ))}
                </View>
              </View>
  ...
  return (
    ...
    >
      <EmptyState
        ...
        onAction={canManage ? () => router.push(Routes.sheepBatchCreate(orgId)) : undefined}
      />
    </View>
  );
}
