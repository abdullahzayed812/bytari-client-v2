...
import { useSheepBatches } from '../hooks';
import type { SheepBatch } from '../types';

/** Route `/(app)/livestock/sheep/[organizationId]/batches` — a FARM's sheep batches. Mirrors `PoultryFlocksScreen`. */
export default function SheepBatchesScreen() {
  ...
  const q = useSheepBatches(orgId);
  const goToDetail = (b: SheepBatch) => router.push(Routes.sheepBatchDetail(orgId, b.id));
  const goToCreate = () => router.push(Routes.sheepBatchCreate(orgId));
