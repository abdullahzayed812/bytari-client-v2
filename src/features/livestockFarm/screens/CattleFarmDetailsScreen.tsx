...
export default function CattleFarmDetailsScreen() {
  ...
  const activeBatches = useCattleBatches(orgId, { status: 'ACTIVE', pageSize: 1, enabled: canOperateFarm });
        ...
        {canOperateFarm ? (
          <View style={{ rowGap: theme.spacing.sm }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.sm }}>
              {sectionCards.map((s) => (
                <FarmSectionCard key={s.key} icon={s.icon} label={s.label} onPress={() => router.push(Routes.cattleFarmSection(orgId, s.key))} />
              ))}
            </View>
          </View>
        ) : null}
