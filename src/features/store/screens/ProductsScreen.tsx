...
/** Route `/organizations/[organizationId]/products` — a veterinary store's catalogue. */
export default function ProductsScreen() {
  ...
  const q = useProducts(orgId, { search, productType: type });

  const goToDetail = (p: Product) => router.push(Routes.organizationProduct(orgId, p.id));
  const goToCreate = () => router.push(Routes.organizationProductCreate(orgId));
    ...
    <SafeAreaScreen>
      ...
      {q.isLoading ? (
        ...
        <FlatList
          data={q.products}
          keyExtractor={(p) => p.id}
          renderItem={({ item }) => <ProductCard product={item} onPress={() => goToDetail(item)} />}
