...
import { AdjustStockForm } from '../components';
import { PRODUCT_STATUS_TONE } from '../constants';
import { useAdjustStock, useDeleteProduct, useProduct, useUpdateProduct } from '../hooks';
import { storeErrorMessage, type AdjustStockFormValues } from '../validation/schemas';

/**
 * Route `/organizations/[organizationId]/products/[productId]`. Sections map 1:1
    ...
    return (
      <ScrollScreen>
        ...
        <EmptyState
          ...
          onAction={() => router.replace(Routes.organizationProducts(orgId))}
        />
      </ScrollScreen>
    );
  }
  ...
  const reactivate = () => {
    ...
    update.mutate(
      { productId: product.id, body: { status: 'ACTIVE' } },
      {
        ...
        onError: (error) => toast.show({ tone: 'danger', message: storeErrorMessage(error, t) }),
      },
    );
  };

  const submitStock = (values: AdjustStockFormValues) => {
    ...
    stock.mutate(
      ...
      },
      {
        ...
        onError: (error) => {
          ...
          setStockError(storeErrorMessage(error, t));
        },
        onSettled: () => {
          ...
          {canManage ? (
            <>
              <Button
                ...
                disabled={busy}
                onPress={() => router.push(Routes.organizationProductEdit(orgId, product.id))}
              />
            ...
            onConfirm={() => {
              ...
              del.mutate(
                { productId: product.id },
                {
                  ...
                  onError: (error) =>
                    toast.show({ tone: 'danger', message: storeErrorMessage(error, t) }),
                },
              );
            }}
