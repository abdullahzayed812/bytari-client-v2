...
export interface FarmStatusCardProps {
  ...
  /** `undefined` until the farm is ACTIVE and the backend has resolved it. */
...
/**
 * The farm owner's single, unified status card (spec §3) — covers BOTH the
 * approval state (`Organization.status`) and, once ACTIVE, the subscription
 * state. Deliberately one card, not two, so the owner never sees redundant or
...
export function FarmStatusCard({
  ...
  // NOT_STARTED (or subscription info not applicable/loaded yet) — the farm is
