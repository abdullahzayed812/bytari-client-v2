...
export function useAdminTraderApplications(
  ...
  >({
    ...
    initialPageParam: 1,
    queryFn: ({ pageParam }) => poultryMarketApi.traders.adminList(params.status, pageParam, pageSize),
...
export function useTraderDecisionMutation(): UseMutationResult<unknown, unknown, TraderDecisionInput> {
  ...
  return useMutation({
    ...
    mutationFn: (input: TraderDecisionInput) => {
      ...
      if (input.decision === 'reject') return poultryMarketApi.traders.reject(input.userId, input.reason);
      if (input.decision === 'suspend') return poultryMarketApi.traders.suspend(input.userId, input.reason);
