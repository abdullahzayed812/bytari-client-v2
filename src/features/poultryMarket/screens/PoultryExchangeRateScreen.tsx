...
/** Route `/(app)/poultry/exchange-rates` — بورصة الدواجن (viewer). */
export default function PoultryExchangeRateScreen() {
  ...
  const date = today();
  const q = usePoultryRates(date);

  return (
    <ScrollScreen>
      <AppHeader
        title={t('exchangeRates.poultryTitle')}
        showBack
        right={
          canManage ? (
            <IconButton
              ...
              onPress={() => router.push(Routes.poultryExchangeRatesEntry)}
            />
          ) : undefined
        }
      />
      ...
      {q.isLoading ? (
        <Section spacing="xl">
          <Loading label={t('exchangeRates.poultryTitle')} />
        </Section>
      ) : q.isError ? (
        <Section spacing="xl">
        ...
        <>
          ...
          <Section spacing="xxl">
            <ExchangeRateTable board="POULTRY" poultryEntries={q.data} showTrend />
          </Section>
        </>
      )}
    </ScrollScreen>
